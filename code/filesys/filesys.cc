...
//this the function that creates the directories
int FileSystem::createDirectory(char *name) {

    int oldSector = this->CurrentDir()->getCurrentSector();

    Directory *currentDir = this->CurrentDir();

    if (currentDir->Find(name) != -1) {//si le dossier contient deja un fichier de même nom
       printf("impossible de créer le répertoire le dossier existe deja\n");
    }
    else{

    BitMap *Map = new BitMap(NumSectors);
    Map->FetchFrom(freeMapFile);
    int freeSector = Map->Find();
    if (freeSector == -1) {
        printf("plus de secteurs libres\n");
    }
    else{
        ...
        newDirHeader->Allocate(Map, -1 * DirectoryFileSize);
